(function(){
  ...
  window.eventImagePreview=function(input){const file=input?.files?.[0],box=document.getElementById('eventImagePreview');if(!box)return;if(!file){box.innerHTML='';return}const url=URL.createObjectURL(file);box.innerHTML=`<div class="event-image-frame"><img src="${url}" alt="공지 이미지 미리보기"></div>`};
